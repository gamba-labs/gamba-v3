...
export function PoolCharts({ pool }: { pool: UiPool }) {
  ...
  const { data: dailyVolume = [], isLoading: isLoadingDailyVolume } = useApi<DailyVolume[]>('/daily', {
    pool: pool.publicKey,
  })
  const {
    data: ratioData = [],
    isLoading: isLoadingRatioData,
  } = useApi<RatioData[]>('/ratio', {
    pool: pool.publicKey,
  })

  const totalVolume = React.useMemo(() => dailyVolume.reduce((prev, x) => prev + x.total_volume, 0), [dailyVolume])
  ...
  const chart = React.useMemo(() => {
    ...
    if (chartId === 'liquidity') {
      return {
        isLoading: isLoadingRatioData,
        data: ratioData.map(({ date, pool_liquidity }) => ({ date, value: pool_liquidity })),
      }
    }
    return {
      isLoading: isLoadingRatioData,
      data: ratioData.map(({ date, pool_liquidity, lp_supply }) => ({
        date,
        value: lp_supply ? pool_liquidity / lp_supply : 1,
      })),
    }
  }, [chartId, dailyVolume, isLoadingDailyVolume, isLoadingRatioData, ratioData])
  ...
  return (
    <Card>
      <Flex justify="between" align="start">
        <Flex direction="column" gap="2">
          <Text size="7" weight="bold">
            {chartId === 'liquidity' && <TokenValue2 exact mint={pool.underlyingTokenMint} amount={hovered?.value ?? Number(pool.liquidity)} />}
            {chartId === 'volume' && <TokenValue2 exact mint={pool.underlyingTokenMint} amount={hovered?.value ?? totalVolume} />}
            {chartId === 'price' && (
              <>
      ...
      <div style={{ height: '200px' }}>
        {chart.isLoading ? <SkeletonBarChart bars={60} /> : <LineChart chart={chart} onHover={hover} lineColor="#8280ff" />}
      </div>
    </Card>
  )
}
