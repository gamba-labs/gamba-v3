            ...
            <UnstyledNavLink key={i} to={"/player/" + player.user}>
              <Card>
                <Flex gap="4">
                  ...
                  <Flex gap="2" justify="between" style={{ flexGrow: 1 }}>
...
export default function Dashboard() {
  ...
  return (
    <Flex direction="column" gap="4">
      ...
      <Flex justify="between">
        <Text color="gray">{showAllPools ? 'All Pools' : 'Top 10 Pools'}</Text>
        <Link
          href="#"
          onClick={(event) => {
            event.preventDefault()
            setShowAllPools((value) => !value)
          }}
        >
          {showAllPools ? 'Show top 10' : 'View all'}
        </Link>
      </Flex>
      <PoolList limit={showAllPools ? Infinity : 10} />
