...
const Inner = withTooltip<AreaProps, any>(
  ({
    ...
    const handleTooltip = React.useCallback(
      ...
      [data, dateScale, onHover, showTooltip, valueScale],
    )
          ...
          {tooltipData && (
            <g>
              <Line
                ...
                strokeDasharray="5,2"
              />
              <circle cx={tooltipLeft} cy={tooltipTop} r={4} fill={lineColor} stroke="white" strokeWidth={1} pointerEvents="none" />
            </g>
          )}
        </svg>
      </div>
...
export function LineChart(props: Omit<AreaProps, 'width' | 'height'>) {
  return <ParentSize debounceTime={250}>{(parent) => <Inner {...props} width={parent.width} height={parent.height} />}</ParentSize>
}
