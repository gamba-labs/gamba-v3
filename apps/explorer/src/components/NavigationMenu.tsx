...
export const ListItemLink = ({ children, title, ...props }: React.PropsWithChildren<{title: string, href: string}>) => (
  <li>
    <NavigationMenu.Link asChild>
      <a className="ListItemLink" target="_blank" rel="noreferrer noopener" {...props}>
        <Flex direction="column">
          <Text>{title}</Text>
          <Text color="gray" size="1">{children}</Text>
        </Flex>
        ...
        <NavigationMenu.Item>
          ...
          <NavigationMenu.Content className="NavigationMenuContent">
            <ul className="List">
              ...
              <ListItemNavLink title="Users" to="/users">
                Inspect active user game accounts
              </ListItemNavLink>
              <ListItemNavLink title="Debug" to="/debug">
